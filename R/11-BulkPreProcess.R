...
create_example_data <- function(n_genes = 20000, n_samples = 12, seed = 123) {
    set.seed(seed)

    gene_names <- paste0("Gene_", sprintf("%05d", seq_len(n_genes)))
    sample_names <- paste0("Sample_", seq_len(n_samples))

    # Efficiently generate simulated data
    counts <- matrix(
    ...
    diff_idx <- sample(n_genes, n_genes * 0.1) # 10% differential genes
    treatment_idx <- seq(n_samples / 2 + 1, n_samples)
    counts[diff_idx, treatment_idx] <- counts[diff_idx, treatment_idx] *
        sample(
            c(0.5, 2),
            length(diff_idx) * length(treatment_idx),
            replace = TRUE,
            prob = c(0.3, 0.7)
        )
    ...
    sample_info <- data.frame(
        ...
        batch = rep(paste0("Batch", 1:3), length.out = n_samples),
        stringsAsFactors = FALSE
    )

    return(list(counts = counts, sample_info = sample_info))
}
...
if (FALSE) {
    ...
    # Full analysis
    filtered_counts1 <- BulkPreProcess(
        data = test_data$counts,
        sample_info = test_data$sample_info,
        check = TRUE,
        show_plot_results = TRUE,
    ...
    filtered_counts2 <- BulkPreProcess(
        data = test_data$counts,
        sample_info = test_data$sample_info,
        check = FALSE,
        gene_symbol_conversion = FALSE
    ...
    filtered_counts3 = SymbolConvert(filtered_counts2)

    # Check dimensions
    cat("Original dimensions:", dim(test_data$counts), "\n")
    cat("Filtered dimensions:", dim(filtered_counts1), "\n")
}
