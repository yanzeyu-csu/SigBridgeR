...
tensor_train = {
    xs: np.concatenate([np.squeeze(Xsc[train_sc2,]), np.squeeze(Xpat[train_pat2, :])]),
...
for i in range(train_steps + 1):
    ...
    if i % 50 == 0:
        print(
            str(sess.run(loss, feed_dict=tensor_train))
            + " "
            + str(sess.run(lossLabel2, feed_dict=tensor_train))
            + "	"
            + str(sess.run(lossMMD, feed_dict=tensor_train))
        )
        # train_sc = resample(50,Ysc,idx_sc)			# CHANGED
        train_pat = idx_pat  # CHANGED
