...
For some reason (maybe r session is contaminated), `reticulate` cannot find the relevant environment. One solution is to pass in the Python path of the environment instead of the environment name.
...
For example, if I want to use the `r-reticulate-degas` environment, I can pass its Python location to `reticulate::use_condaenv()`.
...
```{python,check_tensorflow,eval=FALSE}
...
```{shell,check_tensorflow_shell,eval=FALSE}
