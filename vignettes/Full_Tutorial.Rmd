...
-   [Full Tutorial for SigBridgeR](#full-tutorial-for-sigbridger)
    ...
    -   [6. Other function details](#6-other-function-details)
        ...
        -   [6.3 Setting up Python Environment](#63-setting-up-python-environment)
        -   [6.4 Finding the Optimal Number of Principle Components](#64-finding-the-optimal-number-of-principle-components)
    -   [7. Troubleshooting](#7-troubleshooting)
    -   [8. References](#8-references)

### 0.1 Introduction to SigBridgeR

SigBridgeR (short for **Sig**nificant cell-to-phenotype **Bridge** in **R**) is an R package for screening cells highly associated with phenotype data using single-cell RNA-seq, bulk expression and sample_related phenotype data at a pan-cancer level. It integrates functionality from these R packages: [sunduanchen/Scissor](https://github.com/sunduanchen/Scissor), [Qinran-Zhang/scAB](https://github.com/Qinran-Zhang/scAB/), [WangX-Lab/ScPP](https://github.com/WangX-Lab/ScPP),[aiminXie/scPAS](https://github.com/aiminXie/scPAS) and [tsteelejohnson91/DEGAS](https://github.com/tsteelejohnson91/DEGAS).
...
**This method is not recommended because of intractable dependence control.**
...
This code chunk will CREATE a conda environment called "**r-reticulate-degas**" with python 3.9.15, and install the required packages for DEGAS. If an environment with the same name already exists, it will directly use this environment without creating a new one (unless specified `env_params = list(env.recreate=TRUE)`).

You can use `ListPyEnvs()` to list all the python environments in your system, including virtual environments. Both Windows and Unix-like systems are supported. More information can be found in [Section 6.3](#63-setting-up-python-environment)
...
[![pca](example_figures/bulk_preprocess_pca.png){fig-align="center" width="400"}]((https://github.com/WangLabCSU/SigBridgeR/blob/main/vignettes/example_figures/bulk_preprocess_pca.png))
...
Generally, a larger alpha means stricter screening, which may result in fewer cells being selected. The cells shown in the example can essentially be considered false positives.
...
[![upset](example_figures/upset.png){fig-align="center" width="700"}]((https://github.com/WangLabCSU/SigBridgeR/blob/main/vignettes/example_figures/upset.png))
...
[![umaps](example_figures/umaps.png){fig-align="center" width="400"}]((https://github.com/WangLabCSU/SigBridgeR/blob/main/vignettes/example_figures/umaps.png))
