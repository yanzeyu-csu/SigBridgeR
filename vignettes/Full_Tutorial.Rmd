...
-   [Full Tutorial for SigBridgeR](#full-tutorial-for-sigbridger)
    ...
    -   [6. Other function details](#6-other-function-details)
        ...
        -   [6.3 Detect Rows with Zero Variance in Matrices](#63-detect-rows-with-zero-variance-in-matrices)
...
This single-cell RNA data is from humans. We set the `quality_control` and `data_filter` to FALSE, and we set `scale_features` to all genes in order to maximize the flexibility of downstream analyses and capture a broader range of biological signals, so as to avoid insignificant results caused by too small a dataset. 
...
-   For **dense matrices**, uses `rowVars` ([Section 6.2](#62-calculate-the-variance-of-each-row-in-a-matrix)) implementation to compute row variances with efficient NA handling.\
...
```{r,check_0_var_rows_example,eval=FALSE}
